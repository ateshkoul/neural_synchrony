function [cur_body_part_zero,cur_body_part_out] = DAV_outlier_estimation()
%% Dav_Step_1_Prep 
%
...
raw_data_dir = IBS_get_params_analysis_type('no_aggressive_CAR_ASR_10_ICA_appended_trials').raw_data_dir{1,1};
...
% sub_dir = IBS_get_sub_behavior_dir('Video_landmarks',dyad,subject,[conditions{cond} '_' num2str(rep)]);
% sub_dir = [raw_data_dir sub_dir];
% timestamps = IBS_import_video_timestamps(subject,[conditions{cond} '_' num2str(rep)],sub_dir);
...
mat_oi(:,end-1:end) = [];   %last 2 cols contain dyad and subject number. Removed
    ...
    for body_part = 1 : size(mat_oi,2)/2
        ...
        [ xy,zero_xy_frac,Out_xy_frac ] = AK_Giac_Dav_filtering( x, y, samp_rate, 1, 3);
%         [ xy ] = Giac_Dav_filtering( x, y, samp_rate, 1, 3);
        ...
        cur_body_part_zero(dyad_loop,subject+1,cond,rep,body_part) = zero_xy_frac;
        cur_body_part_out(dyad_loop,subject+1,cond,rep,body_part) = Out_xy_frac;
