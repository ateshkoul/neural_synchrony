...
function [insta_correlation,conditions,save_fname,insta_correlation_dyad] = IBS_load_dyad_tf_insta_corr(data_analysis_type,Dyads,selected_conditions,analysis_sub_type)
%% Atesh
% 23-12-2020
...
for Dyad = 1:length(Dyads)
    ...
    save_fname = [analysis_save_dir sprintf(['Dyd_%0.1d' analysis_sub_type '.mat'],Dyads(Dyad))];
        ...
        for block = 1:length(cur_blocks)
            ...
            switch(analysis_sub_type)
                ...
                case '_insta_50'
                    windowSize_cols = 50;
                    insta_correlation_condition_cur_block{block} = smoothdata(insta_correlation_condition_cur_block{block},smooth_dim,'movmean',windowSize_cols);
                case {'_insta','_insta_abs_corr','_insta_abs_detrend'}
%                     insta_correlation_condition_cur_block{block} = smoothdata(insta_correlation_condition_cur_block{block},smooth_dim,'lowess',10);

                    % do no smoothing
                case '_insta_abs_detrend_mov_1s'
%                     windowSize_cols = 10;
              case '_insta_abs_detrend_movmean_acausal_brain'
                  insta_correlation_condition_cur_block{block} = movmean(insta_correlation_condition_cur_block{block},[0 2]);
              case '_insta_abs_detrend_movmean_causal_brain'
                  insta_correlation_condition_cur_block{block} = movmean(insta_correlation_condition_cur_block{block},[1 0]);

%                     insta_correlation_condition_cur_block{block} = movingmean(insta_correlation_condition_cur_block{block},windowSize_cols,2,[]);
            end
