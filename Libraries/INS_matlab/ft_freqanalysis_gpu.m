function [freq] = ft_freqanalysis_gpu(cfg, data)
...
for itrial = 1:ntrials
  ...
  if itrial == 1
    ...
    elseif keeprpt == 2 % cfg.keeptrials, 'yes' &&  cfg.keeptapers, 'no'
        % Atesh
      if powflg && usegpu, powspctrm     = gpuArray(nan(ntrials,nchan,nfoi,ntoi,cfg.precision));                                                                 end

      if powflg && ~usegpu, powspctrm     = nan(ntrials,nchan,nfoi,ntoi,cfg.precision);                                                                 end
...
if usegpu
    freq.powspctrm = gather(freq.powspctrm);
end
