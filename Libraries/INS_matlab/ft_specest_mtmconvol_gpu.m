function [spectrum,ntaper,freqoi,timeoi] = ft_specest_mtmconvol_gpu(dat, time, varargin)
...
if polyorder >= 0
  dat = ft_preproc_polyremoval_gpu(dat, polyorder, 1, ndatsample);
end
...
switch dimord
  ...
  case 'chan_time_freqtap' % memory efficient representation
    ...
    % Atesh
    if usegpu
    spectrum = gpuArray(complex(zeros([nchan ntimeboi sum(ntaper)])));
    else
    ...
    for ifreqoi = 1:nfreqoi
      ...
      for itap = 1:ntaper(ifreqoi)
        ...
        % Atesh
        if usegpu
        tmp = gpuArray(complex(nan(nchan,ntimeboi),nan(nchan,ntimeboi)));
        else
...
if usegpu
spectrum = gather(spectrum);
ntaper = gather(ntaper);
freqoi = gather(freqoi);
timeoi = gather(timeoi);
