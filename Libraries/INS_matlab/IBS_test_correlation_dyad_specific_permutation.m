function [avg_pseudo_correlations,conditions,pseudo_correlation_fname] = IBS_test_correlation_dyad_specific_permutation(analysis_type,Dyads)
%IBS_TEST_CORRELATION_DYAD_SPECIFIC test whether correlation values are dyad specific
%
% SYNOPSIS: IBS_test_correlation_dyad_specific
%
% INPUT
%
% OUTPUT
...
pseudo_correlation_fname = strrep(correlation_fname,'trialwise_correlations','pseudo_permute_trialwise_correlations');
pseudo_correlation_fname_temp = strrep(correlation_fname,'trialwise_correlations','temp_pseudo_permute_trialwise_correlations');
...
for Dyd_no = 1:length(Dyads)
    ...
    sprintf('Dyd0.2%d',Dyads(Dyd_no))
    for other_Dyd_no = 1:length(other_Dyads)
