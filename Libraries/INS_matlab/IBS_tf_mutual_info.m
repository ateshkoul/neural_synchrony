function [mutual_info,conditions,correlation_fname]= IBS_tf_mutual_info(analysis_type,Dyads)
...
correlation_fname = strrep(correlation_fname,'trialwise_correlations_','trialwise_mutual_info_');


if exist(correlation_fname,'file')
    load(correlation_fname,'mutual_info','conditions');
else
    ...
    parfor Dyd_no = 1:length(Dyads)
        ...
        mutual_info{1,Dyd_no} = cellfun(@(x,y) IBS_power_mutual_info(x,y),data_ica_clean_S1_tf,data_ica_clean_S2_tf,...
            'UniformOutput', false);
    end
    save(correlation_fname,'mutual_info','conditions');
