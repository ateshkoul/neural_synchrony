...
function [stat_main_effects,stat_interaction,cluster_stats_freqwise_fname] = IBS_power_stats_freqwise(data_analysis_type,analysis,cor_fun,varargin_table)
%% Atesh
...
cluster_main_effects_freqwise_fname = IBS_get_params_analysis_type(data_analysis_type,analysis).cluster_main_effects_freqwise_fname{1,1};

cluster_interaction_freqwise_fname = IBS_get_params_analysis_type(data_analysis_type,analysis).cluster_interaction_freqwise_fname{1,1};

cluster_stats_freqwise_fname = IBS_get_params_analysis_type(data_analysis_type,analysis).cluster_stats_freqwise_fname{1,1};
%% cluster based


cluster_main_effects_freqwise_fname = IBS_update_stat_fname(cluster_main_effects_freqwise_fname,data_analysis_type,cor_fun,varargin_table);
cluster_interaction_freqwise_fname = IBS_update_stat_fname(cluster_interaction_freqwise_fname,data_analysis_type,cor_fun,varargin_table);
...
if exist(cluster_stats_freqwise_fname,'file')
    ...
    load(cluster_stats_freqwise_fname,'stat_main_effects','stat_interaction','anova_cond','data_analysis_type','cluster_freq_table');
...
%     [correlations_cell1,cond] = IBS_tf_correlations_trialwise(1,analysis_type);
    ...
    save(cluster_stats_freqwise_fname,'stat_main_effects','stat_interaction','anova_cond','data_analysis_type','cluster_freq_table');
...
cellfun(@(x,y) IBS_cluster_plot(x,y,[],varargin_table.plot_type),stat_main_effects,cluster_main_effects_freqwise_fname)
stat_interaction.stat = -stat_interaction.stat;
IBS_cluster_plot(stat_interaction,cluster_interaction_freqwise_fname,[],varargin_table.plot_type)
close all
